...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (data.success) {
          ...
          setRecentReviews(feedbacks);

          const avgRating =
            feedbacks.length > 0
              ? (
                  feedbacks.reduce((sum, r) => sum + r.rating, 0) /
                  feedbacks.length
                ).toFixed(1)
              : 0;

          setStats({
            users: 1248, // optional, replace with real users if API available
            reviews: feedbacks.length,
            rating: avgRating,
            qr: 278, // optional, replace with backend QR count
          });

          // Monthly chart data (dummy for now, can calculate from feedback createdAt)
          setMonthlyData([
            { month: "Jan", reviews: 40, rating: 4.3, scans: 30 },
            { month: "Feb", reviews: 55, rating: 4.5, scans: 50 },
            { month: "Mar", reviews: 70, rating: 4.7, scans: 65 },
    ...
    // Polling every 10 seconds to auto-update recent reviews
  ...
  const handleGenerateQR = async () => {
    ...
    try {
      ...
      setUniqueId(id);
      const origin = typeof window !== "undefined" ? window.location.origin : "http://localhost:3000";
      const qrLink = `${origin}/feedbackForm/${id}`;

      try {
        await fetch(`${API_BASE}/upload-qr`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ qrId: id, qrLink, createdAt: new Date().toISOString() }),
        });
      } catch (err) {
        console.warn("upload-qr failed (backend)", err);
      }
  ...
  const handleOpenFeedback = () => {
    if (!uniqueId) return;
    router.push(`/feedbackForm/${uniqueId}`);
  };
                ...
                {loading ? (
                  ...
                  recentReviews.map((r) => (
                    <motion.div
                      ...
                      className="border rounded-md p-3 hover:bg-indigo-50 transition-all duration-200"
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          <div className="font-medium">{r.name}</div>
                        ...
                        <div className="text-sm text-gray-600">ID #{r._id}</div>
                      </div>
                      <p className="text-sm mt-2 text-gray-700">{r.comments}</p>
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black/10 backdrop-blur-sm z-50 p-4">
            <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md text-center relative">
              ...
              <button
                onClick={handleGenerateQR}
                disabled={qrLoading}
                className={`px-6 py-3 rounded-xl text-white font-semibold ${qrLoading ? "bg-gray-400" : "bg-blue-600 hover:bg-blue-700"}`}
              >
                {qrLoading ? "Generating..." : "Generate QR"}
              </button>
              ...
              {qrImage && (
                <div className="mt-6">
                  ...
                  <p className="text-gray-500 text-sm mt-1">Scan this QR to open Feedback Page</p>
                  <div className="flex gap-3 justify-center mt-4">
                    <button onClick={handleDownloadQR} className="px-5 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg shadow">
                      Download QR
                    </button>
                    <button onClick={handleOpenFeedback} className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg shadow">
                      Open Feedback Page
                    </button>
                  </div>
                </div>
